...
class ChatBot(object):
    ...
    def get_response(self, statement=None, **kwargs):
        """
        Return the bot's response based on the input. We change it to get the input and the response
        ...
        extraResponse.append(response)  #Second item is the response generated
    ...
    def generate_response(self, input_statement, additional_response_selection_parameters=None):
        ...
        for adapter in self.logic_adapters:
            if adapter.can_process(input_statement):
                ...
                if (output.confidence * (self.storage.getAvgRateById(output.id)/5))/2 > max_confidence: #A normalized confidence that uses levinstein confidence and avg_rating
                    result = output
