...
#Table to associate the Tags with the Statements
...
#The tags from the training files are stored here
class Tag(Base):
    """
    ...
    name = Column(
        String(myconstants.TAG_NAME_MAX_LENGTH),
        unique=True
    )

#All the Statements are stored here
class Statement(Base, StatementMixin):
    ...
    text = Column(
        String(myconstants.STATEMENT_TEXT_MAX_LENGTH)
    )
    ...
    conversation = Column(
        String(myconstants.CONVERSATION_LABEL_MAX_LENGTH),
        nullable=False,
        server_default=''
    )
    ...
    persona = Column(
        String(myconstants.PERSONA_MAX_LENGTH),
        nullable=False,
        server_default=''
    )
    ...
    weight = Column (
        Float,
        default = myconstants.DEFAULT_WEIGHT
    )

    #Average_Rate
    avg_rate = Column (
        Float,
        default = myconstants.DEFAULT_AVG_RATE
    )

    #RatesCounter
    rates_counter = Column (
        Integer,
        default = myconstants.DEFAULT_RATES_COUNTER
    )
