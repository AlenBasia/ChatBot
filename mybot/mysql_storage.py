...
class SQLStorageAdapter(StorageAdapter):
    ...
    def get_statement_model(self):
        ...
        """
        from mybot.mydbmodels import Statement
        return Statement
    ...
    def get_tag_model(self):
        ...
        """
        from mybot.mydbmodels import Tag
        return Tag
    ...
    def create_database(self):
        ...
        """
        from mybot.mydbmodels import Base
    ...
    '''Additional methods for editting the extra attributes we added at the Statement table'''

    def updateWeight(self, id, additional_weight):
        '''
        ...
        mystatement = session.execute('SELECT * FROM statement WHERE id="'+ str(id) +'"').fetchall()
        #get the current weight
        current = mystatement[0]['weight']
        #calculate new weight
        new = current + additional_weight
        #max is 1 min is 0
        if new > 1:
            new = 1
        if new < 0:
        ...
        #update db with the new weight
        session.execute('UPDATE statement SET weight="'+ str(new) +'" WHERE id="' + str(id) + '"')
        session.commit()

        session.close()

    def setMaxWeight(self):
        '''Sets all weights to max'''

        Statement = self.get_model('statement')
        
        session = self.Session()

        session.execute('UPDATE statement SET weight="'+ str(1.0) +'"')
        session.commit()

        session.close()

    def updateRating(self, id, rate):
        '''
        ...
        #Calculate the new avg_rate and rates_counter
        #get the statement by id
        mystatement = session.execute('SELECT * FROM statement WHERE id="'+ str(id) +'"').fetchall()
        #get the current avg_rate
        current_avg = mystatement[0]['avg_rate']
        #get the current rates_counter
        current_counter = mystatement[0]['rates_counter']

        #new counter
        new_counter = current_counter + 1
        #new avg_rate
        new_avg = ((current_avg * current_counter) + rate) / new_counter
        
        #update avg_rate
        session.execute('UPDATE statement SET avg_rate="'+ str(new_avg) +'" WHERE id="' + str(id) + '"')
        session.commit()

        #update rates_counter
        session.execute('UPDATE statement SET rates_counter="'+ str(new_counter) +'"WHERE id="' + str(id) + '"')
        session.commit()
        
        session.close()

    def submitRate(self, id, rate):
        '''
        ...
        #call the method to handle the avg_rate and rates_counter
        self.updateRating(id,rate)

        if rate == 1:
            self.updateWeight(id,-0.2) #if rate is 1 then remove/subtract 0.2 weight
        elif rate == 2:
            self.updateWeight(id,-0.1) #if rate is 2 then remove/subtract 0.1 weight
        elif rate == 4:
            self.updateWeight(id,0.1) #if rate is 4 then add 0.1 weight
        elif rate == 5:
            self.updateWeight(id,0.2) #if rate is 5 then add 0.2 weight
        #if the rate is 3 we dont change the weight


    def getWeightById(self, id):
        '''
        ...
        #Get the Statement by id
            mystatement = session.execute('SELECT * FROM statement WHERE id="'+ str(id) +'"').fetchall()
            result = mystatement[0]['weight']
        
        return result

    def getAvgRateById(self, id):
        '''
        ...
        #Get the Statement by id
            mystatement = session.execute('SELECT * FROM statement WHERE id="'+ str(id) +'"').fetchall()
            result = mystatement[0]['avg_rate']

        return result

    def getRatesCounterById(self, id):
        '''
